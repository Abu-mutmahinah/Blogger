...
const directoryPath = path.join(__dirname, "../MarkdownPart/weeklyContents");
let weeklyPostlist = [];
  ...
  fs.readdir(directoryPath, (err, files) => {
    ...
    files.forEach((file, i) => {
      ...
      fs.readFile(`${directoryPath}/${file}`, "utf8", (arr, contents) => {
        ...
        weeklyPostlist.push(post);
        if (i === files.length - 1) {
          let data = JSON.stringify(weeklyPostlist);
          fs.writeFileSync("src/weeklyPosts.json", data);
        }
      });
    });
  });
